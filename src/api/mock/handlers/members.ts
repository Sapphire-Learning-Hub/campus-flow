...
import type { ManageableProjectRole, ProjectMemberInput, ProjectMemberPatch } from "@/types/member";
import { getProjectPermissions, PERMISSION_DENIED } from "@/utils/Permissions.ts";

const MANAGEABLE_ROLES = new Set<ManageableProjectRole>(["admin", "member", "readonly"]);
...
export const memberHandlers = [
  http.get("/api/members", async ({ request }) => {
    ...
    return ok(mockDatabase.members.filter((member) => memberIds.has(member.id)));
  }),
  ...
  http.post("/api/projects/:projectId/members", async ({ params, request }) => {
    ...
    const member = mockDatabase.members.find((item) => item.id === values.memberId);
  ...
  http.patch("/api/projects/:projectId/members/:memberId", async ({ params, request }) => {
    await delay(220);
    const user = findAuthorizedUser(request);
    if (!user) return fail("请先登录", 401);
    ...
    const projectMember = project.members.find((item) => item.memberId === String(params.memberId));
  ...
  http.delete("/api/projects/:projectId/members/:memberId", async ({ params, request }) => {
    await delay(180);
    const user = findAuthorizedUser(request);
    if (!user) return fail("请先登录", 401);
    ...
    const index = project.members.findIndex((item) => item.memberId === memberId);
